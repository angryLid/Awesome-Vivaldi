...
- **Clean-slate invariant.** TidyTabs never runs `create-new-group` over tabs still registered in another stack — it dismantles unnamed stacks first and excludes named-stack members from its pool. The bridge enforces the same invariant inside `groupTabs`: if every member belongs to one stack, that stack is dissolved via native `unstack` before anything else; members spanning multiple stacks fail with `GROUPED_TABS`. Create-new-group over model-registered members (dual membership, tree updates suppressed by `do-not-reparent`) is a native path nothing in Vivaldi exercises, and its lazy normalization is the prime suspect for delayed first-click desyncs and members escaping to other groups.
- **Group creation is aligned end-to-end with TidyTabs' pipeline.** Adjacency pre-pass (per-member `chrome.tabs.move`, callback raced with a 500 ms timeout since move callbacks are untrusted, contiguity verified via the trusted read path) → `tabsPrivate.move` with contiguous ids → 100 ms settle → `setGroupProperties` → per-member tree metadata written sequentially with 50 ms pacing. Vivaldi's tab model is a tree keyed by `vivExtData.ext_id` / `parent_ext_id`; TidyTabs rewrites the chain explicitly (first member is the root, all others point at it) after every grouping. Earlier builds skipped the tree fields and fed scattered ids to the native move: the stack worked at first, but the latent tree inconsistency was normalized later by Vivaldi (activation reordering, session sync), and the desync then surfaced — first click inside the stack jumped and reverted, and a member could escape its group.
